...
public class Testes
{
	...
	public void criarData()
	{
		...
		assertEquals(dia, catorzeMarço2017.getDayOfMonth());
		assertEquals(DateTimeConstants.MARCH, catorzeMarço2017.getMonthOfYear());
		assertEquals(ano, catorzeMarço2017.getYear());
	}
	...
	@Test
	public void criarPeríodo()
	{
		Period período = new Period(2, 3, 7, 5, 8, 2, 1, 9);
		assertEquals(2, período.getYears());
		assertEquals(3, período.getMonths());
		assertEquals(7, período.getWeeks());
		assertEquals(5, período.getDays());
		assertEquals(8, período.getHours());
		assertEquals(2, período.getMinutes());
		assertEquals(1, período.getSeconds());
		assertEquals(9, período.getMillis());
	}
